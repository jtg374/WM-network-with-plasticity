...
K=1e3;dRi_ = dRi;dRi_(dRi>K)=K; % set an upper bound for plasticity
if any( (t>TStimOff).* (t<TDelayOff) )
    %
    dMEE= -1/param.TJ * param.fMEE(RE,dRe_);
    dMEI=  1/param.TJ * param.fMEE(RI,dRe_);
    dMIE= -1/param.TJ * param.fMEE(RE,dRi_);
    dMII=  1/param.TJ * param.fMEE(RI,dRi_);
    %
else 
    dMEE=zeros(N,N);
    dMEI=zeros(N,N);
...
dy=[dIt;dIw;dRe;dRi;dSee;dSie;dSei;dSii;
reshape(dMEE,N*N,1);reshape(dMEI,N*N,1);reshape(dMIE,N*N,1);reshape(dMII,N*N,1)];  