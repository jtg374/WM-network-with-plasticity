function NDF_with_Plasticity_Frameworks(a,lr,nTrial,r_target)
% clc;clear all;close all;    
datapath = ['/gpfsnyu/scratch/jtg374/WM_Plasticity/inhibitoryPlasticity/' 'UniformP' num2str(a*100) 'LR' num2str(lr) datestr(now,'_yymmdd_HH_MM') 'Trial' num2str(nTrial) ];
mkdir(datapath)
disp(datapath)
mkdir([datapath '/FullData'])
mkdir([datapath '/ActFigures'])
%% load parameters
param = NDF_with_Plasticity_Parameters(a,lr,nTrial,r_target)
...
y0 = [0;              % Stimlus Current Strength
      ...
      reshape(MEI,nx*nx,1) % E to E Connection Strength
      ones(nx,1) % gains
      ]; 
...
save([datapath,'/results.mat'],'RE_readout','MEIt');
